...
public class GetSimplePhotoActivity  extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(this.getIntent()!=null&&this.getIntent().hasExtra(KEY_CROP_PARAM)){
            cropParam=(CropParam) this.getIntent().getSerializableExtra(KEY_CROP_PARAM);
            needCrop=true;
        }
        if (!IntentUtil.isBundleEmpty(getIntent())) {
            ...
            if (bundle.getInt(KEY_FROM_WAY, VALUE_FROM_ALBUM) == VALUE_FROM_ALBUM) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    GetSimplePhotoUtil.choicePicFromCamera_N(this, tempPicFile, RequestCode.CAMERA_OK);
                }else{
        ...
        switch (requestCode) {
            ...
            case RequestCode.ALBUM_OK:
                ...
                if (data != null) {
                    ...
                    if(needCrop){
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                            GetSimplePhotoUtil.cropPic_N(this,cropParam,uri,saveCropUri, RequestCode.CROP_OK);
                        }else{
                            GetSimplePhotoUtil.cropPic(this,cropParam,uri,saveCropUri, RequestCode.CROP_OK);
                        }
                    }else{
