...
public class GetSimplePhotoHelper {
    ...
    @IntDef({GetSimplePhotoHelper.FROM_ALBUM, GetSimplePhotoHelper.FROM_CAMERA})
     ...
     */
    public void choicePhoto(@from final int way, String picFilePath, OnSelectedPhotoListener listener,CropParam cropParam) {
        mFromWay = way;
     ...
     * 启动相机的activity
     */
    private void choicePhotoFromCamera(String picFilePath,CropParam cropParam) {
        Intent intent = new Intent(mActivity, GetSimplePhotoActivity.class);
    ...
    protected void getSelectedPhoto(Uri uri) {
        ...
        try {
            ...
            if(scrPath.startsWith("content")){
                scrPath=GetSimplePhotoUtil.getPath(mActivity,uri);
            }
            ...
            final SimplePhoto photo = new SimplePhoto();
            photo.bitmap = bitmap;
            photo.uri = uri;
